...
router.get("/patients", auth, async (req, res) => {
  try {
    ...
    const patients = await User.find({
      role: "patient"
    }).select("_id firstName lastName email phone age gender");
...
router.get("/patients/:id", async (req, res) => {
  try {
    ...
    const p = await User.findById(id).select("_id firstName lastName email phone age gender medicalHistory");
